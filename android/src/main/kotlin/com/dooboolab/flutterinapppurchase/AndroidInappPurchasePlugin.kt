        ...
        billingClient!!.queryProductDetailsAsync(
            ...
            try {
                ...
                for (skuDetails in skuDetailsList) {
                    ...
                    } else if (skuType == BillingClient.ProductType.SUBS) {
                        val subs = JSONArray()
                        if(skuDetails.subscriptionOfferDetails != null) {
                            for (it in skuDetails.subscriptionOfferDetails!!) {
                                if (it.offerId != null) {
                                    // offer
                                    item.put("freeTrialPeriodAndroid", it.pricingPhases.pricingPhaseList.first().billingPeriod)
                                } else {
                                    // base plan
                                    item.put("price", ((it.pricingPhases.pricingPhaseList.last().priceAmountMicros ?: 0) / 1000000f).toString())
                                    item.put("currency", it.pricingPhases.pricingPhaseList.last().priceCurrencyCode)
                                    item.put("localizedPrice", it.pricingPhases.pricingPhaseList.last().formattedPrice)
                                    item.put("originalPrice", ((it.pricingPhases.pricingPhaseList.last().priceAmountMicros ?: 0) / 1000000f).toDouble())
                                    item.put("introductoryPrice", it.pricingPhases.pricingPhaseList.last().formattedPrice)
                                    item.put("subscriptionPeriodAndroid", it.pricingPhases.pricingPhaseList.last().billingPeriod)
                                    item.put("introductoryPriceCyclesAndroid", it.pricingPhases.pricingPhaseList.last().billingCycleCount)
                                }
                            }
                        }
                    }
